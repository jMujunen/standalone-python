        ...
        for root, dirs, files in os.walk(directory):
            for file in files:
                if file.endswith(old_extention):
                    os.rename(os.path.join(root, file), os.path.join(root, file).replace(old_extention, new_extention))
    else:
        for file in os.listdir(directory):
            if file.endswith(old_extention):
                os.rename(os.path.join(directory, file), os.path.join(directory, file).replace(old_extention, new_extention))
