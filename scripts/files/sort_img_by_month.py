...
def rename_file(output_dir, image_object):
    ...
    if capture_date:
        new_name = f"{capture_date}{image_object.suffix}"
...
def wedding_photos(output_dir, image_object):
    ...
    wedding_photo_path = os.path.join(wedding_photo_dir, image_object.name)
    try:
...
def screenshots(output_dir, image_object):
    ...
    screenshot_path = os.path.join(screenshots_dir, image_object.name)
    try:
...
def rm_empty_folders(folder: str | Path | Path) -> None:
...
def remove_item(file: File, other: File) -> None:
    ...
    #         print(f"Removed {other.name} due to identical EXIF data")
    # elif isinstance(file, Video):
    #     # If the file is a video, compare the metadata
    #     if file.metadata == other.metadata:
    #         os.remove(other.path)
    #         print(f"Removed {other.name} due to identical metadata")
    # else:
    #     # If the file is neither an image nor a video, delete it
    #     os.remove(other.path)
    #     print(f"Removed {other.name} as it is neither an image nor a video
...
def main(input_dir: str | Path, output_dir: str | Path):
    with ExecutionTimer():
        duplicates = []
        ...
        with ProgressBar(num_items) as progress:
            ...
            for item in d.file_objects:
                progress.increment()
                if item.suffix in JUNK or isinstance(item, Git):
                    os.remove(item.path)
                    continue
                # ============================ PHOTO =============================== #
                if item.is_image and isinstance(item, Img):
                    prefix = os.path.join(output_dir, "Photos")
                    if JUNK_FILE_REGEX.match(item.name):
                        try:
                            os.remove(item.path)
                            removed += 1
                    ...
                    try:
                        ...
                        if not item.capture_date:
                            ...
                            no_meta_data_image = os.path.join(prefix, item.name)
                            ...
                            while os.path.exists(no_meta_data_image):
                                ...
                                try:
                                    new_file_name = f"{item.name[:-4]}_{count}{item.suffix}"
                                    shutil.move(
                                        item.path,
                        ...
                        # on metadata date
                            ...
                            output_file_path = os.path.join(prefix, str(capture_year), item.name)
                            ...
                            while os.path.exists(output_file_path):
                                ...
                                try:
                                    ...
                                    output_file_name = f"{filename}_{count}{item.suffix}"
                ...
                # ============================ VIDEO =============================== #
                    ...
                    if not item.capture_date:
                        ...
                        no_meta_data_path = os.path.join(prefix, item.name)
                        count = 0
                        ...
                        capture_year = str(item.capture_date.year)
                        output_file_path = os.path.join(prefix, capture_year, item.name)
                        # Check if the file already exists in the destination directory,
                        # if so increment a counter to create a unique name for it.
                        count = 0
                        while os.path.exists(output_file_path):
                ...
                # ============================ FILE =============================== #
                    ...
                    output_file_path = os.path.join(prefix, item.name)
                    count = 0
                    while os.path.exists(output_file_path):
                ...
                elif not item.is_dir() and not isinstance(item, Dir) and item.suffix is not None:
                    # ============================ MISC  =============================== #
                    ...
                    try:
                        ...
                        # ):  # " #or os.path.is_empty:
                        #     cprint(
                        #         f"Error concatating file paths for file {item.name}",
                        #         fg.red,
                        #         style.bold,
                        #         style.underline,
                        #     )
