...
def parse_arguments():
    ...
    parser.add_argument("successfully_compressed",
                        help="File path to save list of successfully compressed files")
...
def main(input_directory, output_directory):
    ...
    for i, input_file in enumerate(os.listdir(input_directory)):
        print(f"[\033[38;5;200m Processing {
              i + 1} out of {number_of_files} \033[0m]")
        if input_file.endswith(".mp4"):
            ...
            try:
                ...
                output_file = os.path.join(
                    output_directory, f"{file_name}.mp4")
            except Exception as e:
                print(f"[\033[31m {e} \033[0m")
                output_file = os.path.join(
                    output_directory, f"{file_name}1.mp4")
            ...
            if result.returncode == 0:
                print(f"[\033[38;5;200m File {input_file} \
                    successfully converted to {output_file} \033[0m]")
                successfully_compressed.append(input_file)
            else:
                print(f"[\033[38;5;200m File {input_file} could not be converted. \
                    Error code: {result.returncode}:{result.stderr} \033[0m]")
...
if __name__ == "__main__":
    ...
    write_to_file(args.successfully_compressed,
                  files_to_remove)
