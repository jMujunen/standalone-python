        ...
        if exportToHtml:
            if newFile and len(newFile) > 0:
                line = line.replace(
                    ...
                    + ancor
                    + '">'
                    + newFile.replace("\\", "/").split("/")[-1].replace(".md", "")
                    + ancor
                    + "</a>",
                )
            else:  ##self ref
                line = line.replace(
                    ...
                    + ancor
                    + '">'
                    + fileOnly.replace("\\", "/").split("/")[-1].replace(".md", "")
                    + ancor
                    + "</a>",
                )
    return line
    ...
    for size, imglink in re.findall(pattern, line):
        ...
        if exportToHtml:
            if "http" not in imglink:
                ...
                imglink = str(
                    copyFileToExport(
                        unquote(imglink.replace("\\", "/").split("/")[-1]), currentFile
                    )
                )
...
def findCodeBlock(line, InCodeBlock):
    if "```" in line:
        if not InCodeBlock:
            line = (
                '<pre class="codeblock"><code style="tab-size: 4;" class="'
                + line.split("```")[-1].replace("shell\n", "sh").replace("\n", "")
                + ' codeblock">'
                + line.replace("```", "")
            )
        else:
...
def findHeadings(line):
    ...
    for tab, heading, text in re.findall(pattern, line):
        line = (
            ...
            + str(len(tab) * 20)
            + 'px;" id="'
            + text.strip().replace(" ", "_").replace("(", "").replace(")", "")
            + '">'
            + text
            + "</h"
            + str(len(heading))
...
def findInlineCodeBlocks(line):
    ...
    for text in re.findall(pattern, line):
        line = line.replace(
            "`" + text + "`",
            '<code class="inlineCoed">' + html.escape(text) + "</code>",
        )
    return line
...
def readFilesRecursive(path):
    ...
    if exportToHtml:
        with open(
            os.path.join(exportDir, str(path) + ".html"), "w", encoding="utf-8"
        ) as outputfile:
            ...
            outputfile.write(
                '\t<iframe src="'
                + str(findRelPath(".", path))[:-1]
                + 'treeview.html" width="340px" frameBorder="0" height="900px"></iframe>\n'
            )
            outputfile.write("</div>\n")
